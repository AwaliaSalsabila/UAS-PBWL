  <!-- Table Area Starts -->
  <section id="book" class="py-32 px-4 md:px-32 bg-no-repeat bg-center bg-cover w-full relative z-1 text-center" style="background-image: url({{ $tableBg }})">
    ...
    <div class="m-4 max-w-md mt-5 p-4 text-center text-white mx-auto">
      <h3 class="font-bold font-cursive-merie text-4xl leading-normal capitalize">Book <span class="text-amber-400 leading-snug">tempat</span> mu</h3>
      <p class="pt-2 text-[14px] font-sans-lato text-slate-100 leading-relaxed">Nikmati berbagai masakan khas dengan cita rasa luar biasa. Setiap hidangan kami disajikan dengan bahan segar dan cinta.</p>
    </div>

    <form action="{{ route('reservation.store') }}"  method="post" enctype="multipart/form-data" class="bg-slate-800/75 border-2 inline-block mx-5 md:mx-0 px-1.5 md:px-6 py-4 pb-5 md:py-8 md:pb-8 shadow-md">
      @csrf
